...
var ge = class {
  ...
  async uploadFile({ filePath: e3, cloudPath: t3, fileType: n2 = "image", onUploadProgress: s4, config: r2 }) {
    if ("string" !== g(t3))
    ...
    const i3 = r2 && r2.envType || this.config.envType, o2 = (await this.getOSSUploadOptionsFromPath({ env: i3, filename: t3 })).result, a4 = "https://" + o2.cdnDomain + "/" + o2.ossPath, { securityToken: c3, accessKeyId: u3, signature: h4, host: l2, ossPath: d4, id: p2, policy: f2, ossCallbackUrl: m4 } = o2, y4 = { "Cache-Control": "max-age=2592000", "Content-Disposition": "attachment", OSSAccessKeyId: u3, Signature: h4, host: l2, id: p2, key: d4, policy: f2, success_action_status: 200 };
    if (c3 && (y4["x-oss-security-token"] = c3), m4) {
      const e4 = JSON.stringify({ callbackUrl: m4, callbackBody: JSON.stringify({ fileId: p2, spaceId: this.config.spaceId }), callbackBodyType: "application/json" });
      y4.callback = de.toBase64(e4);
    }
    const _2 = { url: "https://" + o2.host, formData: y4, fileName: "file", name: "file", filePath: e3, fileType: n2 };
    if (await this.uploadFileToOSS(Object.assign({}, _2, { onUploadProgress: s4 })), m4)
      return { success: true, filePath: e3, fileID: a4 };
...
class Xe {
  constructor(e3) {
    this.config = e3, this._cache = Re(e3.env), this._request = Ye(e3.env);
  }
  setRefreshToken(e3) {
    const { accessTokenKey: t3, accessTokenExpireKey: n2, refreshTokenKey: s4 } = this._cache.keys;
    this._cache.removeStore(t3), this._cache.removeStore(n2), this._cache.setStore(s4, e3);
...
class Ze {
  constructor(e3) {
    ...
    this._envId = e3, this._cache = Re(this._envId), this._request = Ye(this._envId), this.setUserInfo();
  }
  linkWithTicket(e3) {
    if ("string" != typeof e3)
...
class st extends Xe {
  async signIn(e3, t3) {
    ...
    if (r2)
      return this.setRefreshToken(r2), i3 && o2 ? this.setAccessToken(i3, o2) : await this._request.refreshAccessToken(), await this.refreshUserInfo(), Fe(Me), Fe(je, { env: this.config.env, loginType: We.EMAIL, persistence: this.config.persistence }), new et(this.config.env);
...
class rt extends Xe {
  async signIn(e3, t3) {
    ...
    if (r2)
      return this.setRefreshToken(r2), o2 && i3 ? this.setAccessToken(o2, i3) : await this._request.refreshAccessToken(), await this.refreshUserInfo(), Fe(Me), Fe(je, { env: this.config.env, loginType: We.USERNAME, persistence: this.config.persistence }), new et(this.config.env);
    throw s4.code ? new ne({ code: s4.code, message: `用户名密码登录失败: ${s4.message}` }) : new ne({ message: "用户名密码登录失败" });
  }
}
class it {
  constructor(e3) {
    this.config = e3, this._cache = Re(e3.env), this._request = Ye(e3.env), this._onAnonymousConverted = this._onAnonymousConverted.bind(this), this._onLoginTypeChanged = this._onLoginTypeChanged.bind(this), De(je, this._onLoginTypeChanged);
  }
  get currentUser() {
  ...
  async signOut() {
    ...
    return this._cache.removeStore(e3), this._cache.removeStore(t3), this._cache.removeStore(n2), Fe(Me), Fe(je, { env: this.config.env, loginType: We.NULL, persistence: this.config.persistence }), r2;
  }
...
const ot = function(e3, t3) {
  t3 = t3 || ve();
  const n2 = Ye(this.config.env), { cloudPath: s4, filePath: r2, onUploadProgress: i3, fileType: o2 = "image" } = e3;
  ...
  t3 = t3 || ve();
  const n2 = Ye(this.config.env), { cloudPath: s4 } = e3;
  return n2.send("storage.getUploadMetadata", { path: s4 }).then((e4) => {
    t3(null, e4);
  }).catch((e4) => {
    t3(e4);
  ...
  return Ye(this.config.env).send("storage.batchDeleteFile", n2).then((e4) => {
  ...
  return Ye(this.config.env).send("storage.batchGetDownloadUrl", s4).then((e4) => {
  ...
  return Ye(this.config.env).send("functions.invokeFunction", c3).then((e4) => {
    if (e4.code)
      o2(null, e4);
    else {
...
class Kn {
  ...
  toJSON() {
    ...
    return { $db: t3.reverse().map((e4) => ({ $method: e4.$method, $param: qn(e4.$param) })) };
  }
  toString() {
  ...
  get isAggregate() {
    let e3 = this;
    for (; e3; ) {
      const t3 = Mn(e3), n2 = Mn(e3.prevStage);
  ...
  getNextStageFn(e3) {
    const t3 = this;
    return function() {
      return jn({ $method: e3, $param: qn(Array.from(arguments)) }, t3, t3._database);
    };
  }
  get count() {
  ...
  _send(e3, t3) {
    ...
    if (s4.$db.push({ $method: e3, $param: qn(t3) }), b2) {
...
function jn(e3, t3, n2) {
  return Ln(new Kn(e3, t3, n2), { get(e4, t4) {
    ...
    return e4 && e4.content && (s4 = e4.content.$method), Fn(s4, t4) ? jn({ $method: t4 }, e4, n2) : function() {
      return jn({ $method: t4, $param: qn(Array.from(arguments)) }, e4, n2);
    };
  } });
}
...
function $n(e3, t3 = {}) {
  return Ln(new e3(t3), { get: (e4, t4) => Fn("db", t4) ? jn({ $method: t4 }, null, e4) : function() {
    return jn({ $method: t4, $param: qn(Array.from(arguments)) }, null, e4);
  } });
}
class Wn extends class {
  ...
  _callCloudFunction({ action: e3, command: t3, multiCommand: n2, queryList: s4 }) {
    ...
    function a4(e4) {
      return i3._callback("error", [e4]), q2(M3(o2, "fail"), e4).then(() => q2(M3(o2, "complete"), e4)).then(() => (r2(null, e4), Y2(B2, { type: z2, content: e4 }), Promise.reject(e4)));
    }
    const c3 = q2(M3(o2, "invoke")), u3 = this._uniClient;
...
详细信息：${s6}`), r3(i4);
      ...
      e4.result.errCode = e4.result.errCode || e4.result.code, e4.result.errMsg = e4.result.errMsg || e4.result.message, s5 && c4 && (oe({ token: s5, tokenExpired: c4 }), this._callbackAuth("refreshToken", [{ token: s5, tokenExpired: c4 }]), this._callback("refreshToken", [{ token: s5, tokenExpired: c4 }]), Y2(W, { token: s5, tokenExpired: c4 }));
      ...
      return function(e5) {
        return q2(M3(o2, "success"), e5).then(() => q2(M3(o2, "complete"), e5)).then(() => {
          r2(e5, null);
          const t5 = i3._parseResult(e5);
          return Y2(B2, { type: z2, content: t5 }), Promise.resolve(t5);
        });
      }(e4);
    }, (e4) => {
...
const { loginPage: ns, routerNeedLogin: ss, resToLogin: rs, needLoginPage: is, notNeedLoginPage: os, loginPageInTabBar: as } = function({ pages: e3 = [], subPackages: n2 = [], uniIdRouter: s4 = {}, tabBar: r2 = {} } = t2) {
  const { loginPage: i3, needLogin: o2 = [], resToLogin: a4 = true } = s4, { needLoginPage: c3, notNeedLoginPage: u3 } = Qn(e3), { needLoginPage: h4, notNeedLoginPage: l2 } = function(e4 = []) {
    const t3 = [], n3 = [];
    return e4.forEach((e5) => {
      const { root: s5, pages: r3 = [] } = e5, { needLoginPage: i4, notNeedLoginPage: o3 } = Qn(r3, s5);
    ...
    s4 && function(e4 = {}) {
      ...
      ee().then(() => {
        ...
        if (n3 && hs({ redirect: n3 }))
          return t4.length > 0 ? Y2($, Object.assign({ uniIdRedirectUrl: n3 }, e4)) : void (ns && ls({ api: "navigateTo", redirect: n3 }));
      });
    }(n2);
  });
}
  ...
  return function(t3, n2 = {}) {
    ...
    return new Proxy({}, { get: (s5, c3) => function({ fn: e4, interceptorName: t4, getCallbackArgs: n3 } = {}) {
      return async function(...s6) {
        ...
        let i4, o3;
        try {
          return await q2(M3(t4, "invoke"), { ...r3 }), i4 = await e4(...s6), await q2(M3(t4, "success"), { ...r3, result: i4 }), i4;
        } catch (e5) {
          throw o3 = e5, await q2(M3(t4, "fail"), { ...r3, error: o3 }), o3;
        } finally {
          await q2(M3(t4, "complete"), o3 ? { ...r3, error: o3 } : { ...r3, result: i4 });
        }
      };
      ...
      if (a4 && index.hideLoading(), y4 && y4.token && y4.tokenExpired && (oe(y4), Y2(W, { ...y4 })), g2) {
        let e4 = m4;
        if (p2 && o2) {
        ...
        throw n3.detail = l2.result, Y2(B2, { type: H3, content: n3 }), n3;
      }
      return Y2(B2, { type: H3, content: l2.result }), l2.result;
    ...
    return ["callFunction", "uploadFile", "deleteFile", "getTempFileURL", "downloadFile", "chooseAndUploadFile"].forEach((e4) => {
      ...
      t3[e4] = function() {
        ...
        return function(n4) {
          ...
          n4 = n4 || {};
          const { success: o2, fail: a4, complete: c3 } = te(n4), h4 = i3.then(() => s4 ? Promise.resolve() : q2(M3(t4, "invoke"), n4)).then(() => e5.call(this, n4)).then((e6) => s4 ? Promise.resolve(e6) : q2(M3(t4, "success"), e6).then(() => q2(M3(t4, "complete"), e6)).then(() => (r2 && Y2(B2, { type: J, content: e6 }), Promise.resolve(e6))), (e6) => s4 ? Promise.reject(e6) : q2(M3(t4, "fail"), e6).then(() => q2(M3(t4, "complete"), e6)).then(() => (Y2(B2, { type: J, content: e6 }), Promise.reject(e6))));
          if (!(o2 || a4 || c3))
            return h4;
          h4.then((e6) => {
            o2 && o2(e6), c3 && c3(e6), r2 && Y2(B2, { type: J, content: e6 });
          }, (e6) => {
            a4 && a4(e6), c3 && c3(e6), r2 && Y2(B2, { type: J, content: e6 });
          });
        };
      }(t3[e4], e4).bind(t3);
